...
<div class="wrap">
  ...
  <form method="post">
    ...
    <table class="form-table">
        ...
        <td>
          <select name="parent_id" id="parent_id">
            ...
            <?php foreach ($all as $p): ?>
              <option value="<?php echo esc_attr($p->id); ?>" <?php selected($theme->parent_id ?? '', $p->id); ?>>
      ...
      <?php
        $last_desc_vid = !empty($desc_versions) ? end($desc_versions)->vid : 0; reset($desc_versions);
        $last_op_vid   = !empty($op_versions)   ? end($op_versions)->vid   : 0; reset($op_versions);
        $last_akt_vid  = !empty($akt_versions)  ? end($akt_versions)->vid  : 0; reset($akt_versions);
      ?>
      <tr>
        ...
        <td>
          <select id="desc-versions">
            ...
            ?>
            <option value="<?php echo esc_attr($v->vid); ?>" data-description="<?php echo esc_attr($content); ?>" <?php selected($v->vid, $last_desc_vid); ?>>
      ...
      <tr>
        ...
        <td>
          <select id="op-versions">
            ...
            ?>
            <option value="<?php echo esc_attr($v->vid); ?>" data-opinion="<?php echo esc_attr($content); ?>" <?php selected($v->vid, $last_op_vid); ?>>
              <?php echo esc_html(date('Y-m-d H:i', strtotime($v->created_at))).' – '.esc_html($name); ?>
            </option>
            <?php endforeach; ?>
          </select><br>
        ...
        <td>
          <select id="akt-versions">
            <?php foreach ($akt_versions as $v):
              ...
              $name = $user ? $user->display_name : 'User#'.$v->author_id;
            ?>
            <option value="<?php echo esc_attr($v->vid); ?>" <?php selected($v->vid, $last_akt_vid); ?>>
              <?php echo esc_html(date('Y-m-d H:i', strtotime($v->created_at))).' – '.esc_html($name); ?>
            </option>
            <?php endforeach; ?>
          </select>
